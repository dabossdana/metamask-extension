...
export type AssetDetailsTabsOptions = {
  onTabClick: (tabName: string) => void;
  onSupportLinkClick: () => void;
  defaultHomeActiveTabName: string;
};

export const AssetDetailsTabs = (options: AssetDetailsTabsOptions) => {
  ...
  return (
    <Box style={{ flexGrow: '1' }} paddingTop={tabPadding}>
      ...
      >
        <Tab name={t('tokens')} tabKey="tokens" {...tabProps}>
          <Box marginTop={2}>
            <AssetList
        ...
        <Tab name={t('nfts')} tabKey="nfts" {...tabProps}>
          <NftsTab />
          {
            ///: BEGIN:ONLY_INCLUDE_IF(build-main)
            needHelpButtonLink
        ...
        <Tab name={t('activity')} tabKey="activity" {...tabProps}>
          <TransactionList />
          {
            ///: BEGIN:ONLY_INCLUDE_IF(build-main)
            needHelpButtonLink
